        ...
        case "text/csv": {
          ...
          for (const entry of data) {
            ...
            for (let i = 0; i < entry.length; i++) {
              jsonEntry[i < keys.length ? keys[i] : `column${i + 1}`] =
                entry[i];
            }
      ...
      switch (outputFormat.mime) {
          ...
          for (const value of object) {
            text +=
              keys
                ...
                .map((x) =>
                  csvEscape(typeof x == "string" ? x : JSON.stringify(x)),
                )
                .join(",") + "\n";
          }
          break;
        ...
        case "application/xml": {
          ...
          function write(value: any, tagName: string | null = null) {
            ...
            const isXMLTag =
              typeof value._tag == "string" && Array.isArray(value._children); // is serialized XML tag
