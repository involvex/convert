    ...
    for (let bx = 0; bx < blocksWide; bx++) {
      ...
      for (let py = 0; py < 4; py++) {
        for (let px = 0; px < 4; px++) {
          ...
          const alphaNibble =
            (pixelIndex & 1) === 0 ? alphaByte & 0x0f : alphaByte >> 4;
    ...
    switch (format) {
      ...
      case VTF_IMAGE_FORMAT.RGBA1010102: {
        ...
        out[outOffset + 1] = Math.round(
          (((packed >> 10) & 0x3ff) * 255) / 1023,
        );
        out[outOffset + 2] = Math.round(
          (((packed >> 20) & 0x3ff) * 255) / 1023,
        );
...
function decodeVTF(bytes: Uint8Array): DecodedImage {
  ...
  const topImageSize = getImageDataSize(
    header.imageFormat,
    header.width,
    header.height,
  );
  ...
  for (let level = header.mipmapCount - 1; level > 0; level--) {
    ...
    const mipImageSize = getImageDataSize(
      header.imageFormat,
      mipWidth,
      mipHeight,
    );
