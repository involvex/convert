...
export class TraversionGraph {
    ...
    window.supportedFormatCache.forEach((formats, handler) => {
      ...
      fromIndices.forEach((from) => {
        toIndices.forEach((to) => {
          ...
          const fromCategory =
            from.format.category || from.format.mime.split("/")[0];
          const toCategory = to.format.category || to.format.mime.split("/")[0];
          if (fromCategory && toCategory) {
            const fromCategories = Array.isArray(fromCategory)
              ? fromCategory
              : [fromCategory];
            const toCategories = Array.isArray(toCategory)
              ? toCategory
              : [toCategory];
            if (CATEGORY_HARD_SEARCH) {
              ...
              const costs = CATEGORY_CHANGE_COSTS.filter(
                (c) =>
                  fromCategories.includes(c.from) &&
                  toCategories.includes(c.to),
              );
              if (costs.length === 0)
                cost += DEFAULT_CATEGORY_CHANGE_COST; // If no specific cost is defined for this category change, use the default cost
  ...
  public async *searchPath(
    ...
    let fromIndex = this.nodes.findIndex(
      (node) => node.mime === from.format.mime,
    );
    ...
    while (queue.size() > 0) {
      ...
      if (current.index === toIndex) {
        ...
        console.log(
          `Found path at iteration ${iterations} with cost ${current.cost}: ${current.path.map((p) => p.handler.name + "(" + p.format.mime + ")").join(" -> ")}`,
        );
        if (
          simpleMode ||
          !to.handler ||
          to.handler.name === current.path.at(-1)?.handler.name
        ) {
          console.log(
            `Path valid! Yielding path: ${current.path.map((p) => p.format.mime).join(" → ")}`,
          );
          yield current.path;
          pathsFound++;
        }
